...
for(let i = 0; i <= MAX_LEVEL; i++){  //creates buttons and corresponding links for each level
  ...
  button.onclick = () => {
    for (let j = 0; j <= MAX_LEVEL; j++) {
      if (document.getElementById("buttonContainer").children[j].dataset.level != i) {
        document.getElementById("buttonContainer").removeChild(document.getElementById("buttonContainer").children[j]);
        i--;
      }
    }
